...
import com.naocraftlab.skins.compat.v1_20_1.client.mixin.PlayerModelPreviewAccessor;
...
public final class Minecraft1201PreviewModelAnchors implements AutoCloseable {
    private static final ThreadLocal<Minecraft1201PreviewModelAnchors> ACTIVE = new ThreadLocal<>();

    private final Map<ModelPart, ModelPart.Cube> anchors = new IdentityHashMap<>();

    private Minecraft1201PreviewModelAnchors(EntityModel<?> model) {
    ...
    public static Minecraft1201PreviewModelAnchors open(EntityModel<?> model) {
        if (ACTIVE.get() != null) {
            throw new IllegalStateException("Player model anchors are already active");
        }
        Minecraft1201PreviewModelAnchors scope = new Minecraft1201PreviewModelAnchors(model);
        ACTIVE.set(scope);
        return scope;
    }

    public static ModelPart.Cube anchor(ModelPart part) {
        Minecraft1201PreviewModelAnchors scope = ACTIVE.get();
