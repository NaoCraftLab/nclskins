...
public final class Minecraft12111BakedPreviewRenderer
        extends PictureInPictureRenderer<Minecraft12111BakedPreviewRenderState> {
    private final Set<Minecraft12111BakedPreviewTarget> targets =
            Collections.newSetFromMap(new WeakHashMap<>());

    public Minecraft12111BakedPreviewRenderer(
            MultiBufferSource.BufferSource bufferSource) {
        super(bufferSource);
    }

    @Override
    public Class<Minecraft12111BakedPreviewRenderState> getRenderStateClass() {
        return Minecraft12111BakedPreviewRenderState.class;
    }

    @Override
    public void prepare(
            Minecraft12111BakedPreviewRenderState state,
            GuiRenderState guiRenderState,
            int guiScale) {
        Minecraft12111BakedPreviewTarget target = state.target();
    ...
    protected void renderToTexture(
            Minecraft12111BakedPreviewRenderState state, PoseStack pose) {
        throw new IllegalStateException("Dispatcher render target must not be used");
    }

    @Override
    public void close() {
        for (Minecraft12111BakedPreviewTarget target : new ArrayList<>(targets)) {
            target.release(this);
        }
        targets.clear();
        super.close();
    ...
    private static final class TargetRenderer
        ...
        private static void configure(Minecraft12111BakedPreviewRenderState state) {
            PlayerModel player = state.playerModel();
            if (player != null) {
                player.resetPose();
