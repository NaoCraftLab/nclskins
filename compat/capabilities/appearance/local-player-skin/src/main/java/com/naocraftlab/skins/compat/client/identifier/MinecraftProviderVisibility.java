...
import com.naocraftlab.skins.compat.client.identifier.mixin.SkinManagerProviderMixin;
...
public final class MinecraftProviderVisibility {
    ...
    public static CompletableFuture<PlayerSkin> lookup(SkinManager manager, GameProfile profile) {
        ...
        try {
            var service = Minecraft.getInstance().services().sessionService();
            ...
            return ((SkinManagerProviderMixin) manager).nclskins$registerProviderTextures(profile.id(), filtered)
    ...
    static void set(ProviderVisibility visibility) {
        ...
        if (connection != null) {
            for (var info : connection.getOnlinePlayers()) {
                if (!minecraft.getUser().getProfileId().equals(info.getProfile().id())) {
